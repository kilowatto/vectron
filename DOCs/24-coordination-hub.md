...
| 2026-07-26 | Kimi (k3) | **🔴 BUG CRÍTICO RESUELTO — el cubo renderizaba NEGRO para todos desde F1.4a:** el campo líquido usaba **15 vertex buffers** y WebGPU garantiza solo **8** (`Vertex buffer count (15) exceeds the maximum (8)` — el pipeline nunca se creaba; el adapter de Chrome reporta 8 y rechaza pedir 16; el fallback WebGL también moría por exceso de atributos). Fix: empaquetado a **8 exactos** (position+normal+6 vec4: `aHomeScale`, `aColorGain`, `aBodyPhase`, `aSpring`, `aJellyAxisAmp`, `aAnim`); `instanceMatrix` eliminada (`mesh.instanceMatrix = null` — NodeMaterial la inyectaba como 9º buffer), `aFreq` muerta borrada, `aAnimAxis` por hash en shader, highlight+focus fusionados en `aGain`, picking por ray-esfera manual (`pickInstanceAtRay`, ya sin raycast de InstancedMesh). Commits `70a1911` (buffers) + `5350210` (look líquido: radio celda 0.052, tinte env 0.85, core 1.6, baseGlow 0.3, bloom engine 0.42/0.5). Verificado con capturas Chrome real: cubo visible WebGPU + WebGL, picking OK, dominios por color legibles a 25k | `particleField.ts`, `sceneInteraction.ts`, `engine.ts` | Iterar más jugo (metaballs/perf vertex-env) cuando el usuario valide |
