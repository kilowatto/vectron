...
| 2026-07-26 | Kimi (k3) | **F2-drawers DESPLEGADO — F2 COMPLETA en código:** `<vx-drawer>` accesible (aria, Esc, pushState/popstate, ≥44px, reduced-motion) envolviendo zoom rail, nav de superficies, Math Lab y chromeLegend; renombrado R-13 (sceneInteraction, levelSelect/levelSwitcher, mathLab); fixes 18 P0.5 integrados; commit `ecfea94`, live y verificado en workers.dev (`vx-drawer` en bundle servido). **Pendiente:** dominio custom `vectron.kilowatto.com` sirve HTML stale por regla de caché de la zona (ignora query strings); purge API no disponible con token OAuth de wrangler — esperando expiración de TTL | `app/src/ui/components/drawer.*`, renames §10.2, `main.ts`, `style.css`, `i18n.ts` | **CHECKPOINT USUARIO:** validación visual de F2 completa |
